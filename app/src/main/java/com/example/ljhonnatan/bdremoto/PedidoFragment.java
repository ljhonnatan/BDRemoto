...
import com.example.ljhonnatan.bdremoto.R;
...
public class PedidoFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener{
    ...
    ListView lvListado;
    //ArrayList<medico> listaDatos;
    ArrayList<planta> listaDatos;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        listaDatos = new ArrayList<planta>();
    ...
    public class CargarListaProducto extends AsyncTask<Void, Void, Integer> {
        ...
        protected Integer doInBackground(Void... params) {
            Integer retorno = 0;
            try {
                String urlListado = Funciones.URL_WS + "planta.listar.php";
    ...
    private void cargarListaDatos(String resultado) {
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                item.setFecha(jsonData.getString("fecha"));
                //item.setHora(jsonData.getString("hora"));
                item.setCodigoPlanta(jsonData.getInt("codigo_planta"));
                item.setDescripcion(jsonData.getString("descripcion"));
                item.setHora(jsonData.getString("hora"));
                listaDatos.add(item);
            }
        }catch (Exception e){
